...
def create_2d_visualization(palabra1: str, palabra2: str, palabra3: str,
                            v_palabra1: np.ndarray, v_palabra2: np.ndarray,
                            v_palabra3: np.ndarray, v_resultado: np.ndarray,
                            candidatos: Dict[str, np.ndarray]):
    """
    Crea visualización 2D de los vectores usando PCA.
    """
    # Preparar datos para PCA
    labels = [palabra1, palabra2, palabra3, "RESULTADO"] + list(candidatos.keys())
    vectors = [v_palabra1, v_palabra2, v_palabra3, v_resultado] + list(candidatos.values())
    ...
    # Separar puntos
    base_points = X_2d[:3]  # palabra1, palabra2, palabra3
    resultado_point = X_2d[3:4]  # resultado
    candidatos_points = X_2d[4:]  # candidatos

    # Crear figura
    fig = go.Figure()

    # Vectores desde origen (0,0) a las palabras base
    origen = np.array([0, 0])

    # Vector para palabra1 (Madrid)
    fig.add_trace(go.Scatter(
        x=[0, base_points[0, 0]],
        y=[0, base_points[0, 1]],
        mode='lines',
        name=f'Vector {palabra1}',
        line=dict(color='#3498db', width=3),
        showlegend=True
    ))

    # Vector para palabra2 (España)
    fig.add_trace(go.Scatter(
        x=[0, base_points[1, 0]],
        y=[0, base_points[1, 1]],
        mode='lines',
        name=f'Vector {palabra2}',
        line=dict(color='#9b59b6', width=3),
        showlegend=True
    ))

    # Vector para palabra3 (Francia)
    fig.add_trace(go.Scatter(
        x=[0, base_points[2, 0]],
        y=[0, base_points[2, 1]],
        mode='lines',
        name=f'Vector {palabra3}',
    ...
    # Vector resultado
    fig.add_trace(go.Scatter(
        x=[0, resultado_point[0, 0]],
        y=[0, resultado_point[0, 1]],
        mode='lines',
        name='Vector Resultado',
        line=dict(color='#e74c3c', width=4, dash='dash'),
        showlegend=True
    ))

    # Puntos base (palabra1, palabra2, palabra3)
    fig.add_trace(go.Scatter(
        x=base_points[:, 0],
        y=base_points[:, 1],
        mode='markers+text',
        name='Palabras Base',
        marker=dict(size=15, color=['#3498db', '#9b59b6', '#f39c12']),
    ...
    fig.add_trace(go.Scatter(
        ...
        y=resultado_point[:, 1],
        mode='markers+text',
        name='Punto Resultado',
        marker=dict(size=25, color='#e74c3c', symbol='star'),
        text=['RESULTADO'],
        textposition='top center',
    ...
    # Candidatos
    candidatos_labels = list(candidatos.keys())
    fig.add_trace(go.Scatter(
        x=candidatos_points[:, 0],
        y=candidatos_points[:, 1],
        mode='markers+text',
        name='Candidatos',
        marker=dict(size=12, color='#2ecc71', symbol='diamond'),
        text=candidatos_labels,
        textposition='top center',
    ...
    fig.update_layout(
        title=f"Visualización 2D: {palabra1} - {palabra2} + {palabra3}",
        xaxis_title=f"Componente Principal 1 ({pca.explained_variance_ratio_[0]:.1%} var.)",
        yaxis_title=f"Componente Principal 2 ({pca.explained_variance_ratio_[1]:.1%} var.)",
        height=600,
        showlegend=True,
        hovermode='closest',
        plot_bgcolor='#f8f9fa'
